public class renderarea {
  ...
  public int linesL = (int)(materialW / distance / 2);
  public int linesR = linesL;
...
void mouseClicked(float mx, float my) {
  ...
  if (mouseOver && sqrt(sq(mx-(midx-0.6*s))+sq(my-(midy-0.6*s))) < r) {
    linesL--;
    if (linesL < 0) { linesL = 0; }
    click = true;
  }
  if (mouseOver && sqrt(sq(mx-(midx+0.6*s))+sq(my-(midy-0.6*s))) < r) {
    linesR--;
    if (linesR < 0) { linesR = 0; }
    click = true;
  }
  if (mouseOver && sqrt(sq(mx-(midx+1.7*s))+sq(my-(midy-0.6*s))) < r) {
    linesR++;
